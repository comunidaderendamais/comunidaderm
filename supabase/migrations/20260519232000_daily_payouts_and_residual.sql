...
begin
  day_key := ((run_at at time zone 'America/Sao_Paulo')::date)::text;
  ...
  credits as (
    select
      profile_id,
      lot_id,
      round(plan_price * units *
        case plan_key
          when 'cota10' then 0.01::numeric
          when 'cota50' then 0.011::numeric
          when 'cota100' then 0.012::numeric
          else 0::numeric
        end
      , 2) as amount
    from active_lots
  ...
  due as (
    select profile_id, lot_id, amount
    from credits
  ...
  ins_daily as (
    ...
    select
      ...
      jsonb_build_object(
        ...
        'payment', 'SISTEMA',
        'status', 'Creditado',
        'meta', jsonb_build_object('lotId', d.lot_id, 'day', day_key)
      ),
      run_at
    from due d
...
create or replace function public.admin_run_daily_payouts(run_at timestamptz default now())
returns jsonb
language plpgsql
security definer
set search_path = public
...
begin
  ...
  return public.process_daily_payouts(run_at);
end;
$$;

revoke all on function public.process_daily_payouts(timestamptz) from public;
revoke all on function public.admin_run_daily_payouts(timestamptz) from public;
grant execute on function public.process_daily_payouts(timestamptz) to service_role;
grant execute on function public.admin_run_daily_payouts(timestamptz) to authenticated;
