import { ArrowRight, Gift, Sparkles, Trophy, Users } from 'lucide-react';

const InfoCard = ({ icon: Icon, label, value, hint, accentClass }) => (
  <div className="rounded-[24px] border border-white/70 bg-white/95 p-5 shadow-[0_20px_60px_-40px_rgba(15,23,42,0.25)] backdrop-blur">
    ...
    <p className="mt-5 text-2xl font-black tracking-tight text-gray-950">{value}</p>
  </div>
);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 min-[540px]:grid-cols-3">
        <InfoCard
          icon={Trophy}
          label={t.bonusYourCurrentRank}
          value={rankTitle}
          hint={t.bonusMetricRankHint}
          accentClass="border-emerald-100 bg-emerald-50 text-emerald-600"
        />
        <InfoCard
          icon={Users}
          label={t.bonusCurrentVolume}
          value={currentVolume}
          hint={nextRankLabel}
        ...
        <InfoCard
          icon={Gift}
          label={t.bonusYourStatus}
          value={statusLabel}
          hint={t.bonusMetricStatusHint}
